...
Public Class ZipStorer
    ...
    Private Shared ReadOnly MinZipTime As Date = New Date(1980, 1, 1, 0, 0, 0)
...
#Region "Private fields"
    ...
    Private Shared _ownStream As Boolean
#End Region
...
#Region "Public methods"
    ...
    Public Sub Close()
        ...
        If _ownStream AndAlso Me.ZipFileStream IsNot Nothing Then
...
#Region "'Private methods"
    ...
    ' Local file header: local file header signature 4 bytes (&H04034b50) version needed to extract 2
    ' bytes general purpose bit flag 2 bytes compression method 2 bytes last mod file time 2 bytes
    ' last mod file date 2 bytes crc-32 4 bytes compressed size 4 bytes uncompressed size 4 bytes
    ' filename length 2 bytes extra field length 2 bytes
    ...
    ' End of central dir record: end of central dir signature 4 bytes (&H06054b50) number of this
    ' disk 2 bytes number of the disk with the start of the central directory 2 bytes total number of
    ' entries in the central dir on this disk 2 bytes total number of entries in the central dir 2
    ' bytes size of the central directory 4 bytes offset of start of central directory with respect
    ' to the starting disk number 4 bytes zipfile comment length 2 bytes zipfile comment (variable size)
    ...
    ' DOS Date and time: MS-DOS date. The date is a packed value with the following format. Bits
    ' Description 0-4 Day of the month (1–31) 5-8 Month (1 = January, 2 = February, and so on) 9-15
    ' Year offset from 1980 (add 1980 to get actual year) MS-DOS time. The time is a packed value
    ' with the following format. Bits Description 0-4 Second divided by 2 5-10 Minute (0–59) 11-15
    ' Hour (0–23 on a 24-hour clock)

    Private Function DateTimeToDosTime(ByVal _dt As DateTime) As UInteger
        If _dt < MinZipTime Then Return &H0 And &HFFFFFFFF

        Return &HFFFFFFFF And
            ((_dt.Second / 2) Or
             (_dt.Minute << 5) Or
             (_dt.Hour << 11) Or
             (_dt.Day << 16) Or
    ...
    ' CRC32 algorithm The 'magic number' for the CRC is &Hdebb20e3. The proper CRC pre and post
    ' conditioning is used, meaning that the CRC register is pre-conditioned with all ones (a
    ' starting value of &Hffffffff) and the value is post-conditioned by taking the one's complement
    ' of the CRC residual. If bit 3 of the general purpose flag is set, this field is set to zero in
    ' the local header and the correct value is put in the data descriptor and in the central directory.
